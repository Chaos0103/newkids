...
@RestController
@Slf4j
@RequestMapping("/quiz-controller")
public class QuizController {

    @PostMapping("/start")
    public ApiResponse<String> loadingQuiz() {
        //회원 단어장 조회
        //10개 미만이면 예외 발생
        //아니라면 레디스에 등록 후 키 부여
    ...
    @PostMapping("/next")
    public ApiResponse<QuizWordResponse> nextWord() {
        //다음 단어 송출
    ...
    @PostMapping("/answer")
    public ApiResponse<Boolean> checkAnswer(@Valid @RequestBody CheckAnswerRequest request) {
        //정답 확인
        //반환: ture, false
    ...
    @PostMapping("/result")
    public ApiResponse<QuizResultResponse> resultQuiz() {
